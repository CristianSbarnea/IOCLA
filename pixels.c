...
#include <time.h>
#include "pixel.h"
#define grayscale_R 0.3
#define grayscale_G 0.59
#define grayscale_B 0.11
...
void colorToGray(Picture *pic){
	for(int i = 0; i < pic->height; i++)
		for(int j = 0; j < pic->width; j++){
			pic->pix_array[i][j].R *= grayscale_R;
			pic->pix_array[i][j].G *= grayscale_G;
			pic->pix_array[i][j].B *= grayscale_B;
		}
}
